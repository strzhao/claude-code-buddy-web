...
export class CatEntity {
  ...
  update(
    ...
    if (mousePos !== null && this.state !== 'scared'
      && this.state !== 'permissionRequest' && this.state !== 'taskComplete') {
  ...
  private _resetIdleTimeout() {
    this.idleTimer = 0
    this.idleTimeout = PHYSICS.idleMinTime + Math.random() * (PHYSICS.idleMaxTime - PHYSICS.idleMinTime)
  }
